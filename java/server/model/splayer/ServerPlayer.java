...
/**
 * @author Hayden Player holds all data for a specific player in the game. That
 *         data includes the player's hand of cards called a PlayerBank, their
 *         name, the number of victory points they have earned, and their index
 *         in the list of players. This index also functions as an identifier
 *         for each player. This class also implements the methods the Facade
 *         will call to check if a trade can be offered, accepted, or if a
 *         certain constructable can be built.
 */
public class ServerPlayer {
	...
	private int monumentDevs; // The number of monument cards a player holds.
								// Used to determine when the cards can be
								// played.
	private boolean playedDevCard; // True if the player has used a dev card
									// this turn.
	...
	private int remainingRoads; // The number of roads the player has left to
								// build
	private int remainingSettlements; // The number of settlements the player
										// has left to build
	private int soldierDevs; // The number of played soldier cards. Used to
								// determine largest army
	private int victoryPoints; // The player's victory points

	/**
	 * Creates a new Player object with an empty PlayerBank object and zero
	 * victory points
	 * 
	 * @param playerIdx
	 *            Initializes the player's index in the list of players
	 * @param playerName
	 *            The name the user entered upon starting the game.
	 */
	public ServerPlayer(int playerIdx, int playerID, String playerName,
			String color) {
	...
	public ServerPlayer(JsonPlayer jsonPlayer) {
		this.bank = new ServerPlayerBank(jsonPlayer.getResources(),
				jsonPlayer.getOldDevCards(), jsonPlayer.getNewDevCards());
	 ...
	 * @param victoryPoints
	 *            The player's victoryPoints
	 * @param remainingRoads
	 *            The number of roads the player has remaining
	 * @param remainingSettlements
	 *            The number of settlements the player has remaining
	 * @param remainingCities
	 *            The number of cities the player has remaining
	 */
	public void update(ServerPlayerBank bank, int victoryPoints,
			int remainingRoads, int remainingSettlements, int remainingCities) {
		this.bank = bank;
	...
	/**
	 * Called by the Facade to determine if a player has the resources necessary
	 * to offer a certain trade
	 * 
	 * @param playerIdx
	 *            The player that offered the trade
	 * @param offeredRes
	 *            A map of offered resource cards
	 * @return return whether a trade can be offered or not
	 */
	public boolean canOfferTrade(int playerIdx,
			HashMap<ResourceType, Integer> offeredRes) {
		return bank.hasResCards(offeredRes, false);
	}

	/**
	 * Called by the Facade to determine if a player has the resources necessary
	 * to accept a certain trade
	 * 
	 * @param playerIdx
	 *            The player that offered the trade
	 * @param desiredRes
	 *            A list of resource cards desired by the other player
	 * @return return whether a trade was accepted or not
	 */
	public boolean canAcceptTrade(int playerIdx,
			HashMap<ResourceType, Integer> desiredRes) {
		return bank.hasResCards(desiredRes, true);
	}

	/**
	 * @param devCard
	 *            type of dev card player wants to play
	 * @return true if player has that dev card, hasn't already played it, and
	 *         meets the requirement to play that certain dev card.
	 */
	 ...
	 * 
	 * @param isFree
	 *            Whether or not the road is free.
	 */
	...
	/**
	 * Takes a dev card from the player. If it's a soldier, it also increments
	 * their soldierDevs.
	 * 
